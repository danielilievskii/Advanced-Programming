package SecondMidtermExercises.Ex32_GenericMap;
...
class MapOps {
    public static <K extends Comparable<K>, V> Map<K, V> merge(Map<K, V> map1, Map<K, V> map2, MergeStrategy<V> mergeStrategy) {
        ...
        map2.keySet().stream()
                ...
                .forEach(key -> resultMap.put(key, mergeStrategy.execute(map1.get(key), map2.get(key))));
        map1.keySet().stream()
                .filter(key -> !resultMap.containsKey(key))
                .forEach(key -> resultMap.put(key, map1.get(key)));
        map2.keySet().stream()
                .filter(key -> !resultMap.containsKey(key))
                .forEach(key -> resultMap.put(key, map2.get(key)));
