package SecondMidtermExercises.Ex05_GenericCollection;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ...
    public void addGenericItem(String category, T element) {
        elements.putIfAbsent(category, new TreeSet<>());

        if(elements.get(category).contains(element)) {
            return;
        }
        elements.get(category).add(element);

//        elements.computeIfPresent(category, (k, v) -> {
//            v.add(element);
//            return v;
//        });
    }
    public Collection<T> itemsFromCategories(List<String> categories) {
        return elements.keySet().stream()
                ...
                .flatMap(category -> elements.get(category).stream())
    ...
    public Map<String, Set<T>> byMonthAndDay(){
        return elements.values().stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(el -> String.format("%02d-%02d", el.getTimestamp().getMonthValue(), el.getTimestamp().getDayOfMonth()),
                        TreeMap::new,
    ...
    public Map<Integer, Long> countByYear() {
        return elements.values().stream()
                .flatMap(Collection::stream)
                .collect(Collectors.groupingBy(el -> el.getTimestamp().getYear(),
                        TreeMap::new,
                        Collectors.counting()
                ));
    }
