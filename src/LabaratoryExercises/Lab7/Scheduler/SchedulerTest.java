package LabaratoryExercises.Lab7.Scheduler;
...
//        return false;
    ...
    public T next() {
        Date date = new Date();
        return map.get(map.keySet().stream().filter(k -> k.after(date)).min(Date::compareTo).orElse(null));
    }

    public T last() {
        Date date = new Date();
        return map.get(map.keySet().stream().filter(k -> k.before(date)).max(Date::compareTo).orElse(null));
    }

    public ArrayList<T> getAll(Date begin, Date end) {
        return map.entrySet().stream().filter(entry -> entry.getKey().after(begin) && entry.getKey().before(end)).map(Map.Entry::getValue).collect(Collectors.toCollection(ArrayList::new));
    }
