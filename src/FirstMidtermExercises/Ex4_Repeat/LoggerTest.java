package FirstMidtermExercises.Ex4_Repeat;
...
class LogSystem <E extends ILog> {
    ...
    public LogSystem(ArrayList<E> elements) {
    ...
    void printResults() {
        ...
        LogProcessor<E> firstLogProcessor = elements -> {
            return elements.stream().filter(log -> log.getType().equals("INFO")).collect(Collectors.toCollection(ArrayList::new));
        };
        LogProcessor<E> secondLogProcessor = elements ->
            elements.stream().filter(log -> log.getMessage().length() < 100).collect(Collectors.toCollection(ArrayList::new));


        LogProcessor<E> thirdLogProcessor = elements -> elements.stream().sorted().collect(Collectors.toCollection(ArrayList::new));
